...
def md_to_latex(md_text):
    ...
    try:
        # 告知 pandoc 图片文件夹的位置，让它生成正确的相对路径
        # 注意：这里的 extra_args 假设 doc_handler.py 的输出就是 word_md_parts
        # Pandoc 会自动将 `images/` 路径保留
        return pypandoc.convert_text(md_text, 'latex', format='md')
    except Exception:
        return f"% (pandoc convert failed) \n{md_text}"

# ===============================
# ✅ [修改] LaTeX -> SVG (xelatex + dvisvgm)
# ===============================
def latex_full_document_body(user_tex: str, image_base_path: Path):
    """
    把用户的 LaTeX 片段封装成完整的 tex 文档，并正确设置图片路径。
    """
    # 将图片文件夹路径转换为 LaTeX 能理解的、带正斜杠的绝对路径
