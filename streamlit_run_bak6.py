...
class Question(Base):
    ...
    # [cite: 1] 删除了 course, grade, chapter 文本字段，改为外键 ID 字段
    course_id = Column(Integer)  # [cite: 1]
    grade_id = Column(Integer)    # [cite: 1]
    chapter_id = Column(Integer)  # [cite: 1]
    ...
    # [cite: 1] 增加了 quality 字段 (SmallInt 对应 ORM 的 Integer/SmallInteger)
    quality = Column(SmallInteger) # [cite: 1] 使用 SmallInteger 对应 smallint 类型
    created_at = Column(TIMESTAMP, server_default=func.now())
    updated_at = Column(TIMESTAMP, server_default=func.now(), onupdate=func.now())
    
# ===============================
# Helpers: parse files, md->latex (保持不变)
# ===============================
...
# ===============================
# LaTeX -> SVG (xelatex + dvisvgm) (保持不变)
# ===============================
...
with left_col:
    ...
    # [cite: 1] 替换文本输入为 ID 数字输入
    col_ids_1, col_ids_2, col_ids_3 = st.columns(3)
    with col_ids_1:
        course_id = st.number_input("课程 ID (course_id)", min_value=0, value=0, step=1)
    with col_ids_2:
        grade_id = st.number_input("年级 ID (grade_id)", min_value=0, value=0, step=1)
    with col_ids_3:
        chapter_id = st.number_input("章节 ID (chapter_id)", min_value=0, value=0, step=1)
        
    q_type = st.text_input("题目类型（单选/多选/解答）")
    difficulty = st.number_input("难度（1-5）", min_value=1, max_value=5, value=3)
    # [cite: 1] 替换难度为质量 quality 字段
    quality = st.number_input("题目质量 (quality 1-5)", min_value=1, max_value=5, value=3)
    ...
    if st.button("✅ 确认 LaTeX 并写入数据库"):
        ...
        if not st.session_state.get("_latex_preview_success", False):
            ...
            try:
                ...
                # [cite: 1] 使用新的 ID 字段和 quality 字段
                q = Question(
                    title=title or None,
                    content_md=md_text or None,
                    content_latex=latex_text or None,
                    course_id=course_id if course_id > 0 else None,   # [cite: 1]
                    grade_id=grade_id if grade_id > 0 else None,     # [cite: 1]
                    chapter_id=chapter_id if chapter_id > 0 else None, # [cite: 1]
                    ...
                    quality=int(quality) if quality else None         # [cite: 1]
                )
                db.add(q)
                db.commit()
...
with right_col:
    ...
    db = SessionLocal()
    try:
        # [cite: 1] 替换文本过滤为 ID 过滤
        col_filter_1, col_filter_2, col_filter_3 = st.columns(3)
        with col_filter_1:
            course_id_filter = st.number_input("课程 ID 过滤", min_value=0, value=0, key="f_course_id")
        with col_filter_2:
            grade_id_filter = st.number_input("年级 ID 过滤", min_value=0, value=0, key="f_grade_id")
        with col_filter_3:
            chapter_id_filter = st.number_input("章节 ID 过滤", min_value=0, value=0, key="f_chapter_id")
            
        type_filter = st.text_input("题型过滤")
        diff_min, diff_max = st.slider("难度范围", 1, 5, (1,5))
        # [cite: 1] 新增 quality 过滤
        ...
        if course_id_filter > 0:
            filters.append(Question.course_id == course_id_filter)
        if grade_id_filter > 0:
            filters.append(Question.grade_id == grade_id_filter)
        if chapter_id_filter > 0:
            filters.append(Question.chapter_id == chapter_id_filter)
        ...
        if quality_min is not None:
            filters.append(Question.quality >= quality_min) # [cite: 1]
        if quality_max is not None:
            filters.append(Question.quality <= quality_max) # [cite: 1]
        ...
        if "edit_id" in st.session_state:
            ...
            if instance:
                ...
                # [cite: 1] 编辑 UI 替换为 ID 字段
                col_edit_ids_1, col_edit_ids_2, col_edit_ids_3 = st.columns(3)
                with col_edit_ids_1:
                    e_course_id = st.number_input("课程 ID", min_value=0, value=instance.course_id or 0, key="e_course_id")
                with col_edit_ids_2:
                    e_grade_id = st.number_input("年级 ID", min_value=0, value=instance.grade_id or 0, key="e_grade_id")
                with col_edit_ids_3:
                    e_chapter_id = st.number_input("章节 ID", min_value=0, value=instance.chapter_id or 0, key="e_chapter_id")
                ...
                # [cite: 1] 增加 quality 编辑
                e_quality = st.number_input("质量", min_value=1, max_value=5, value=instance.quality or 3)
                ...
                if st.button("💾 保存修改"):
                    try:
                        ...
                        instance.quality = int(e_quality) if e_quality else None # [cite: 1] 保存 quality instance.answer = e_answer or None instance.analysis = e_analysis or None
        ...
        # 确保 session 在查询结束时关闭，但编辑操作会重新打开和关闭
